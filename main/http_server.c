...
/**
 * Checks the g_fw_update_status and creates the fw_update_reset_timer if the g_fw_update_status is true
 */
static void http_server_fw_update_reset_timer(void) {
    if (g_fw_update_status) {
        ESP_LOGI(TAG, "http_server_fw_update_reset_timer: FW updated successful, statring FW update reset timer");

        // Give the web page a chance to receive an acknowledge back and intialize the timer
...
static esp_err_t http_server_OTA_update_handler(httpd_req_t *req) {
    ...
    do {
        ...
        if ((received_length = httpd_req_recv(req, ota_buff, MIN(content_length, sizeof(ota_buff)))) < 0) {
            // Check if timeout occured
            if (received_length == HTTPD_SOCK_ERR_TIMEOUT) {
                ESP_LOGI(TAG, "https_server_OTA_update_handler: Socket timeout");
                continue; // Retry receiving if timeout occurred
            }
            ESP_LOGI(TAG, "http_server_OTA_update_handler: OTA other Error %d", received_length);
            return ESP_FAIL;
        }
        printf("https_server_OTA_update_handler: OTA RX: %d of %d\r", content_received, content_length);

        // Is this the first data we are receiving
        if (!is_req_body_started) {
            is_req_body_started = true;
            // Get the location of the .bin file content (remove the webform data from the request)
            char *body_start = strstr(ota_buff, "\r\n\r\n") + 4;
            int body_part_len = received_length - (body_start - ota_buff);
            ...
            esp_ota_write(ota_handle, body_start, body_part_len);
    ...
    if (flash_successful) http_server_monitor_send_message(HTTP_MSG_OTA_UPDATE_SUCCESSFULL);
    else http_server_monitor_send_message(HTTP_MSG_OTA_UPDATE_FAILED);
...
static httpd_handle_t http_server_configure(void) {
    ...
    config.stack_size = HTTP_SEVER_TASK_STACK_SIZE;
...
BaseType_t http_server_monitor_send_message(http_server_message_e msgID) {
    http_server_queue_msg_t msg = {.msgID =  msgID};
