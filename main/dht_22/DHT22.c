...
#include "../../../../.espressif/tools/xtensa-esp-elf/esp-13.2.0_20240530/xtensa-esp-elf/xtensa-esp-elf/include/sys/time.h"
...
static void dht22_task(void *pvParameter) {
	...
	for (;;) {
		// Using dummy data
		srand(time(0));
		temperature = rand() / (float)RAND_MAX * 30.0;
		humidity = 20.0 + rand() / (float)RAND_MAX * 60.0;
