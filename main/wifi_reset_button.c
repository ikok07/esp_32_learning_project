...
/**
 * WiFi reset button tasks reacts to a BOOT button event by sending a message
...
void wifi_reset_button_task(void *pvParam) {
    for (;;) {
        if (xSemaphoreTake(wifi_reset_semaphore, portMAX_DELAY) == pdTRUE) {
            ...
            // Send the message to disconnect the WiFi and clear credentials
...
void wifi_reset_button_config(void) {
    ...
    // Attach the interrupt service routinge
